...
# Database Connection
engine = create_engine("postgresql://postgres:Tejas%40123@localhost:5432/ncaafb_db")

# Sidebar Navigation
st.sidebar.title("Navigation")
page = st.sidebar.radio(
    "Go to",
    ["🏠 Home", "🧩 Teams Explorer", "👥 Players Explorer", "📅 Seasons", "🏆 Rankings", "📌 Team Profile", "🏟️ Venues", "🧑‍🏫 Coaches", "📊 Analysis"]
)
...
if page == "🏠 Home":
    ...
    with col1:
        ...
        teams_df = pd.read_sql("SELECT market, name, conference_id FROM teams", engine)
        st.dataframe(teams_df)

    with col2:
        st.subheader("Active Players")
        players_df = pd.read_sql("SELECT first_name, last_name, position, status FROM players WHERE status = 'Active' LIMIT 50", engine)
        st.dataframe(players_df)

    with col3:
        st.subheader("Seasons")
        seasons_df = pd.read_sql("SELECT year, start_date, end_date, status FROM seasons", engine)
...
# -------------------- TEAMS PAGE --------------------
    ...
    # Load teams with conference and division
    query = """
    SELECT 
        t.team_id,
        t.market,
    ...
    with col1:
        conf_filter = st.selectbox("Conference", ["All"] + sorted(teams_df["conference"].dropna().unique().tolist()))
    with col2:
        div_filter = st.selectbox("Division", ["All"] + sorted(teams_df["division"].dropna().unique().tolist()))
    with col3:
    ...
    team_selected = st.selectbox("Select a Team", teams_df["name"].unique())
    ...
    roster_df = pd.read_sql(roster_query, engine)
...
# -------------------- PLAYERS PAGE --------------------
    ...
    with col2:
        status_filter = st.selectbox(
            "Status",
            ["All"] + sorted(players_df["status"].dropna().unique().tolist())
        )

    with col3:
        eligibility_filter = st.selectbox(
            "Eligibility Year",
            ["All"] + sorted(players_df["eligibility"].dropna().astype(str).unique().tolist())
        )
    ...
    with col5:
        min_height = st.number_input("Min height", min_value=0, value=0, step=1)

    with col6:
        min_weight = st.number_input("Min weight", min_value=0, value=0, step=1)
    ...
    if eligibility_filter != "All":
        players_df = players_df[players_df["eligibility"].astype(str) == eligibility_filter]

    if search:
        players_df = players_df[
            players_df["first_name"].str.contains(search, case=False) |
            players_df["last_name"].str.contains(search, case=False) |
            players_df["team_name"].str.contains(search, case=False)
        ]
    players_df = players_df[players_df["height"].fillna(0) >= min_height]
    players_df = players_df[players_df["weight"].fillna(0) >= min_weight]
